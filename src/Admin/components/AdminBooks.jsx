...
import { allBooks, getAllUsers } from "../../services/allAPIs";
...
const AdminBooks = () => {
  ...
  const getBookData = async () => {
    try {
      ...
      let apires = await allBooks(header, "");
              ...
              {userData?.length > 0 ? (
                <div className="grid grid-cols-3 gap-6 mx-20 my-10">
                  {userData?.map((eachuser, index) => (
                    <div
                      className="border border-gray-400 bg-blue-500 p-5 text-white  rounded-2xl"
                      key={index}
                    >
                      <div>
                        <img
